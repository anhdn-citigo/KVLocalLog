...
extension FileManager {
    ...
    func createDirectoryIfNeeded(path: URL) {
        if !fileExists(atPath: path.path) {
            do {
                ...
                KVFnbLogHelper.shared.saveLogLocal(eventLog: "Could not create directory at path \(path). Error: \(error)", networkStatus: "current Network status")
            }
        }
    }
    ...
    func clearLogsFolder() {
        ...
        do {
            ...
            KVFnbLogHelper.shared.saveLogLocal(eventLog: "Không xóa được thư mục Logs! Lỗi: \(error)", networkStatus: "current Network status")
